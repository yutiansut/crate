...
public class Union implements LogicalPlan {
    ...
    static Builder create(UnionSelect union, SubqueryPlanner subqueryPlanner, Functions functions, CoordinatorTxnCtx txnCtx) {
        return (tableStats) -> {
            ...
            LogicalPlan lhsPlan = LogicalPlanner.plan(left, subqueryPlanner, true, functions, txnCtx).build(tableStats);
            LogicalPlan rhsPlan = LogicalPlanner.plan(right, subqueryPlanner, true, functions, txnCtx).build(tableStats);
