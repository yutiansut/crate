...
.. SEEALSO::
    ...
    The ``crate-node`` command is modeled on the `elasticsearch-node`_ command.
    For additional information, consult the Elasticsearch documentation.
...
.. _elasticsearch-node: https://www.elastic.co/guide/en/elasticsearch/reference/current/node-tool.html
